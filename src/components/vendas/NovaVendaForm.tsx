...
export default function NovaVendaForm({
    ...
    const handleChangeProduto = useCallback(
        (itemId: number, produtoId: number) => {
            ...
            setItens((prev) =>
                prev.map((item) => {
                    if (item.id === itemId) {
                        // set basic selection, but wait price confirmation
                        return {
                            ...item,
                            produtoId,
            ...
            // Open modal to confirm or edit price
            if (prod) {
    ...
    const handleCancelPrice = useCallback(() => {
        // revert selection for that item
...
// Price confirmation modal render (kept outside main return for readability)
