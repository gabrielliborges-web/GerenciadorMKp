...
export default function ProdutoList({
    ...
    return (
        <div className="hidden md:block overflow-x-auto rounded-2xl border border-white/10 dark:border-primary-600/20 bg-white/5 dark:bg-gradient-to-br dark:from-white/5 dark:to-white/2 backdrop-blur-sm">
            {/* Selection bar */}
            {selectedIds && selectedIds.size > 0 && (
                <div className="border-b border-white/10 dark:border-white/10 p-3 lg:p-4 bg-gradient-to-r from-white/5 to-white/5 dark:from-primary-600/10 dark:to-primary-700/10 flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        <button
                            ...
                            className="flex items-center gap-2 px-3 lg:px-4 py-1.5 lg:py-2 text-xs lg:text-sm font-semibold rounded-lg bg-red-500/20 dark:bg-red-500/20 text-red-400 dark:text-red-400 hover:bg-red-500/30 dark:hover:bg-red-500/30 transition-all duration-300"
                        >
                        ...
                        <button
                            ...
                            className="px-3 lg:px-4 py-1.5 lg:py-2 text-xs lg:text-sm font-semibold rounded-lg bg-white/10 dark:bg-white/10 text-white/70 dark:text-white/70 hover:bg-white/20 dark:hover:bg-white/20 transition-all duration-300"
                        >
                            Cancelar
                        </button>
                    </div>
            ...
            <table className="w-full text-sm">
                <thead>
                    <tr className="border-b border-white/10 dark:border-primary-600/20">
                        <th className="px-4 lg:px-6 py-3 lg:py-4">
                            <input
                                type="checkbox"
                                checked={selectedIds ? selectedIds.size > 0 && selectedIds.size === produtos.length : false}
                                onChange={onSelectAll}
                                className="h-4 w-4 cursor-pointer rounded border-primary-600/30 dark:border-primary-600/30 bg-primary-600/10 dark:bg-primary-600/10 text-primary-400 dark:text-primary-400 transition-colors"
                                title={selectedIds && selectedIds.size === produtos.length ? "Desselecionar todas" : "Selecionar todas"}
                            />
                        </th>
                        <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-white/60 dark:text-white/60">
                            Produto
                        </th>
                        <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-white/60 dark:text-white/60">
                            Categoria
                ...
                <tbody className="divide-y divide-white/10 dark:divide-primary-600/10">
                        ...
                        return (
                            ...
                            <>
                                <tr
                                    ...
                                    className={`group transition-all duration-300 hover:bg-white/5 dark:hover:bg-white/5 cursor-pointer ${rowSelected ? 'bg-primary-600/10 dark:bg-primary-600/10' : ''}`}
                                >
                                    ...
                                    <td className="px-6 py-4 text-sm text-white/80 dark:text-white/80">
                                        {produto.categoria?.nome || "-"}
                                    </td>
                                    <td className="px-6 py-4 text-sm font-semibold text-white dark:text-white">
                                    ...
                                    <td className="px-6 py-4">
                                        {produto.estoque <= 5 ? (
                                            <div className="inline-flex items-center gap-2 rounded-full bg-yellow-500/10 dark:bg-yellow-500/10 px-3 py-1 text-sm font-semibold text-yellow-300 dark:text-yellow-300">
                                                <AlertTriangle className="h-4 w-4 text-yellow-300 dark:text-yellow-300" />
                                                Estoque baixo
                                            </div>
                                        ) : (
                                            <span className="text-sm text-white/60 dark:text-white/60">-</span>
                                        )}
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-2">
                                            <button
                                                ...
                                                className="rounded-lg p-2 transition-all duration-300 hover:bg-primary-600/20 dark:hover:bg-primary-600/20"
                                                title="Detalhes"
                                            >
                                                <Info className="h-4 w-4 text-primary-400 dark:text-primary-400" />
                                            </button>
                                            <button
                                                onClick={(e) => { e.stopPropagation(); onEdit(produto.id); }}
                                                className="rounded-lg p-2 transition-all duration-300 hover:bg-white/10 dark:hover:bg-white/10"
                                                title="Editar"
                                            >
                                                <Edit className="h-4 w-4 text-white/70 dark:text-white/70" />
                                            </button>
                                            <button
                                                onClick={(e) => { e.stopPropagation(); onDelete(produto.id); }}
                                                className="rounded-lg p-2 transition-all duration-300 hover:bg-red-500/20 dark:hover:bg-red-500/20"
                                                title="Excluir"
                                            >
                                ...
                                {isExpanded && (
                                    <tr className="bg-transparent">
                                        <td colSpan={numeroColunas} className="px-6 py-4">
                                            <div className="bg-white/5 dark:bg-white/5 border border-white/10 dark:border-white/10 rounded-xl p-4 mt-2">
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                    {/* Coluna esquerda: resumo do produto e preços */}
                                                    <div>
                                                        <h3 className="text-sm font-semibold text-white dark:text-white">Resumo do produto</h3>
                                                        <p className="mt-2 text-sm text-white/80 dark:text-white/80">{produto.nome}</p>
                                                        <p className="text-xs text-white/60 dark:text-white/60">Categoria: {produto.categoria?.nome || "-"} • Status: {produto.ativo ? "Ativo" : "Inativo"} • Estoque: {produto.estoque}</p>

                                                        <div className="mt-4">
                                                            <h4 className="text-sm font-semibold text-white dark:text-white">Preços & Estoque</h4>
                                                            <div className="mt-2 space-y-1 text-sm text-white/80 dark:text-white/80">
                                                    ...
                                                    <div>
                                                        <h4 className="text-sm font-semibold text-white dark:text-white">Análise financeira</h4>
                                                        <div className="mt-2 space-y-2 text-sm text-white/80 dark:text-white/80">
                                                            <div>
                                                                <div className="text-xs text-white/60 dark:text-white/60">Margem unitária (preço normal)</div>
                                                            ...
                                                            <div>
                                                                <div className="text-xs text-white/60 dark:text-white/60">Margem unitária (promocional)</div>
                                                        ...
                                                        <div className="mt-4">
                                                            <h4 className="text-sm font-semibold text-white dark:text-white">Alertas / Insights</h4>
                                                            <ul className="mt-2 list-disc list-inside text-sm text-white/80 dark:text-white/80 space-y-1">
                                                                {alerts.map((a, idx) => (
                                                                    <li key={idx} className={a.startsWith("⚠️") ? "text-red-300 dark:text-red-300" : "text-white/80 dark:text-white/80"}>{a}</li>
                                                                ))}
                                                            </ul>
                                                        </div>
                                                    </div>
