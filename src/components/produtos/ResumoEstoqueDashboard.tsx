    ...
    return (
        <div className="mb-8">
            <button
                ...
                aria-expanded={open}
                onClick={() => setOpen((v) => !v)}
                className="mb-4 w-full flex items-center justify-between gap-3 rounded-lg border border-white/10 bg-white/3 px-4 py-3 text-left transition hover:bg-white/5"
            >
                <div>
                    <p className="text-lg font-semibold text-white">Resumo do Estoque</p>
                    <p className="text-sm text-white/60 hidden sm:block">Clique para {open ? "ocultar" : "mostrar"} o resumo</p>
                </div>
                <ChevronDown className={`h-5 w-5 transform transition-transform ${open ? "-rotate-180" : "rotate-0"}`} />
            </button>
            ...
            >
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                    <div className="rounded-xl border border-white/10 bg-white/5 p-5">
                        <p className="text-sm font-semibold text-slate-300">Vender todo o estoque (preço normal)</p>
                        <p className="mt-3 text-2xl font-bold text-white">{formatCurrency(totalVenderNormal)}</p>
                        <p className="mt-2 text-sm text-white/60">Baseado em <span className="font-medium">{quantidadeItensEstoque}</span> itens em estoque</p>
                    </div>

                    <div className="rounded-xl border border-white/10 bg-white/5 p-5">
                        <p className="text-sm font-semibold text-slate-300">Vender todo o estoque (preço promocional)</p>
                        <p className="mt-3 text-2xl font-bold text-white">{formatCurrency(totalVenderPromocional)}</p>
                        <p className="mt-2 text-sm text-white/60">Considera preço promocional quando disponível</p>
                    </div>

                    <div className="rounded-xl border border-white/10 bg-white/5 p-5">
                        <p className="text-sm font-semibold text-slate-300">Desconto potencial</p>
                        <p className="mt-3 text-2xl font-bold text-white">{formatCurrency(diferencaPromocional)}</p>
                        <p className="mt-2 text-sm text-white/60">{descontoPercentual > 0 ? `${descontoPercentual.toFixed(2)}% de desconto` : "Sem desconto"}</p>
                    </div>

                    <div className="rounded-xl border border-white/10 bg-white/5 p-5">
                        <p className="text-sm font-semibold text-slate-300">Produtos ativos / Itens</p>
                        <p className="mt-3 text-2xl font-bold text-white">{quantidadeProdutosAtivos} / {quantidadeItensEstoque}</p>
                        <p className="mt-2 text-sm text-white/60">Produtos com estoque baixo: <span className="font-medium">{produtosEstoqueBaixo}</span></p>
                    </div>
                </div>
                ...
                {totalPorCategoria.length > 0 && (
                    <div className="mt-6 rounded-xl border border-white/10 bg-white/5 p-5">
                        <p className="text-sm font-semibold text-slate-300">Valor por categoria (preço normal)</p>
                        <div className="mt-4 space-y-3">
                            {totalPorCategoria.map((cat) => (
                                <div key={cat.key} className="flex items-center gap-4">
                                    <div className="w-44 text-sm text-white/80">{cat.nome}</div>
                                    ...
                                    <div className="w-36 text-right text-sm font-medium text-white">{formatCurrency(cat.total)}</div>
                                </div>
                            ))}
                        </div>
                    </div>
