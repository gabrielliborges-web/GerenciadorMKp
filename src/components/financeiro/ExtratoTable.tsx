...
export default function ExtratoTable({
    ...
    return (
        <div className="space-y-4">
            {/* Desktop Table */}
            <div className="hidden md:block rounded-2xl border border-mauve-light-6 dark:border-white/10 bg-white dark:bg-white/5 overflow-hidden">
                    ...
                    <table className="w-full">
                        <thead className="border-b border-mauve-light-6 dark:border-white/10 bg-mauve-light-2 dark:bg-white/5">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-right text-xs font-semibold text-text-secondary-light dark:text-white/70">
                                    Entrada/Saída
                                </th>
                                <th className="px-6 py-4 text-right text-xs font-semibold text-text-secondary-light dark:text-white/70">
                                    Saldo após
                        ...
                        <tbody>
                            {movimentacoesComSaldo.map((mov, idx) => (
                                <tr
                                    key={mov.id}
                                    className={`border-t border-mauve-light-6 dark:border-white/5 transition-colors hover:bg-mauve-light-2 dark:hover:bg-white/10 ${idx % 2 === 0 ? "bg-transparent" : "bg-mauve-light-1 dark:bg-white/[0.02]"
                                        }`}
                                >
                                    ...
                                    <td className="px-6 py-4">
                                        <span className="inline-flex items-center gap-2 rounded-lg bg-mauve-light-2 dark:bg-white/10 px-3 py-1 text-xs font-semibold text-text-primary-light dark:text-white">
                                            {mov.entrada === true ? (
                                                <>
                                                ...
                                                <>
                                                    <ArrowDownLeft className="h-4 w-4 text-red-700 dark:text-red-400" />
                                                    Saída
                                                </>
                                            )}
                                        </span>
            ...
            <div className="md:hidden space-y-3">
                {movimentacoesComSaldo.map((mov) => (
                    <div
                        key={mov.id}
                        className="rounded-xl border border-mauve-light-6 dark:border-white/10 bg-white dark:bg-white/5 p-4"
                    >
                        <div className="flex items-start justify-between mb-3">
                            <div className="flex-1">
                                ...
                                <p className="text-xs text-text-secondary-light dark:text-white/60">{formatTime(mov.data)}</p>
                            </div>
                        ...
                        <div className="flex items-center justify-between border-t border-mauve-light-6 dark:border-white/10 pt-2">
                            <div>
                                <p className="text-xs text-text-secondary-light dark:text-white/60">Categoria</p>
                                <p className="text-sm text-text-primary-light dark:text-white">{mov.categoria}</p>
                            </div>
                            <div className="text-right">
                                <p className="text-xs text-text-secondary-light dark:text-white/60">Saldo após</p>
                                <p
                                    className={`text-sm font-semibold ${mov.saldoApos >= 0 ? "text-green-700 dark:text-green-400" : "text-red-700 dark:text-red-400"
                                        }`}
                                >
                                    R$ {mov.saldoApos.toFixed(2)}
                                </p>
            ...
            {/* Pagination */}
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between rounded-xl border border-mauve-light-6 dark:border-white/10 bg-white dark:bg-white/5 p-4">
                <div className="flex items-center gap-2">
                    <span className="text-sm text-text-secondary-light dark:text-white/70">Itens por página:</span>
                    <select
                        value={itemsPerPage}
                ...
                <div className="flex items-center justify-between sm:justify-center gap-2">
                    <button
                        ...
                        className="rounded-lg border border-mauve-light-6 dark:border-white/10 bg-mauve-light-2 dark:bg-white/5 p-2 text-text-secondary-light dark:text-white/70 transition-all hover:bg-mauve-light-3 dark:hover:bg-white/10 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        <ChevronLeft className="h-4 w-4" />
                    </button>

                    <span className="text-sm text-text-secondary-light dark:text-white/70">
                ...
                <div className="text-sm text-text-secondary-light dark:text-white/70">
